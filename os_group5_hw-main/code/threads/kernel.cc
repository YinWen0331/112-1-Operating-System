...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
		...
		}else if (strcmp(argv[i], "-ep") == 0) {                                //12-8
            ASSERT(i + 2 < argc);
            execfile[++execfileNum]= argv[++i];
...
Kernel::Initialize()
{
    ...
    usedPhyPage = new UsedPhyPage();        // 11-23 add
...
void Kernel::ExecAll()
{
	for (int i=1;i<=execfileNum;i++) {
		int a = Exec(execfile[i], threadPriority[i]);                           //12-8
	}
...
int Kernel::Exec(char* name, int priority)
{
    ...
    t[threadNum]->setPriority(priority);                       //12-8
...
UsedPhyPage::UsedPhyPage()      //11-23 add
{
...
int UsedPhyPage::numUnused()      //11-23 add
{
    int count = 0;
...
int UsedPhyPage::checkAndSet()      //11-23 add
{
    int unUsedPage = -1;
