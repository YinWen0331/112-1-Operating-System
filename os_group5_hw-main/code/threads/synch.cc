...
Semaphore::P()
{
    ...
    //cout << currentThread->getID() << " ask value:" << value<< "\n";

    while (value == 0) { 		// semaphore not available

        if(currentThread->getPriority() >= 100 && currentThread->getPriority() <= 149){
            if( !Q1->IsInList(currentThread) ){
                Q1->Insert(currentThread);
            }
        }
        else if ( (currentThread->getPriority() >= 50 && currentThread->getPriority() <= 99) ){
            if( !Q2->IsInList(currentThread) ){
                Q2->Insert(currentThread);
            }
        }
        else if ( (currentThread->getPriority() >= 0 && currentThread->getPriority() <= 49) ){
            if( !Q3->IsInList(currentThread) ){
                Q3->Append(currentThread);
            }
        }
...
Semaphore::V()
{
    ...
    if( !Q1->IsEmpty() ){
        kernel->scheduler->ReadyToRun(Q1->RemoveFront());
    }else if ( !Q2->IsEmpty() ){
        kernel->scheduler->ReadyToRun(Q2->RemoveFront());
    }else if ( !Q3->IsEmpty() ){
        kernel->scheduler->ReadyToRun(Q3->RemoveFront());
    } 
