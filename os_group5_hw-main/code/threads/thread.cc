...
Thread::Yield ()
{
    ...
    ASSERT(this == kernel->currentThread);
    //cout << this->getID() <<" in Yield and priority :"<< this->getPriority()<<"\n";
    DEBUG(dbgThread, "Yielding thread: " << name);


    this->setYieldTime(kernel->stats->totalTicks - this->getLastexecutionTime());

    kernel->scheduler->ReadyToRun(this);                    //12-8
...
Thread::Sleep (bool finishing)
{
    ...
    if(this->getTotalExe() != 0){
        ...
        //float prevAprx = this->getAprxBurstTime();
        //float newAprx = 0.5 * prevAprx + 0.5 * this->getExecutionTime();
        //this->setAprxBurstTime(newAprx);
        DEBUG(dbgSche, "[D]Tick[" << kernel->stats->totalTicks << "]: Thread [" << this->getID() << "] update approximate burst time, from: ["
                << prevBurstTime << "], add [" << this->getTotalExe() << "], to [" << newBurstTime << "]");
    }
