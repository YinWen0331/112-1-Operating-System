...
// how big is address space?
    ...
    pageTable = new TranslationEntry[numPages];     // 11-23 add
    for (int i = 0; i < numPages; i++) {        // 11-23 add
    ...
    unsigned int physicalAddr;      // 11-23 add
    int unReadSize;     // 11-23 add
    int chunkStart;     // 11-23 add
    int chunkSize;      // 11-23 add
    int inFilePosiotion;        // 11-23 add
...
// Note: this code assumes that virtual address = physical address
    if (noffH.code.size > 0) {
        ...
        */
        ...
        unReadSize = noffH.code.size;       // 11-23 add
        chunkStart = noffH.code.virtualAddr;        // 11-23 add
        chunkSize = 0;      // 11-23 add
        inFilePosiotion = 0;        // 11-23 add

        /* while still unread code */
        while(unReadSize > 0) {     // 11-23 add
    ...
    if (noffH.initData.size > 0) {
        ...
        */
        ...
        unReadSize = noffH.initData.size;       // 11-23 add
        chunkStart = noffH.initData.virtualAddr;        // 11-23 add
        chunkSize = 0;      // 11-23 add
        inFilePosiotion = 0;        // 11-23 add
    ...
    if (noffH.readonlyData.size > 0) {
        ...
        */
        ...
        unReadSize = noffH.readonlyData.size;       // 11-23 add
        chunkStart = noffH.readonlyData.virtualAddr;       // 11-23 add
        chunkSize = 0;       // 11-23 add
        inFilePosiotion = 0;       // 11-23 add
...
int AddrSpace::calChunkSize(int chunkStart, int unReadSize)       // 11-23 add
{
    int chunkSize;
